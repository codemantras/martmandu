            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                            ...
                            @foreach($categories as $category)
                                <li class="mb-4" wire:key="{{$category->id}}">
                                    <label for="category-{{$category->slug}}"
                                        ...
                                        <input type="checkbox" wire:model.live="selected_category"
                            ...
                            @foreach($brands as $brand)
                                <li class="mb-4" wire:key="{{$brand->id}}">
                                    <label for="brand-{{$brand->slug}}" class="flex items-center dark:text-gray-300">
                                        <input type="checkbox" wire:model.live="selected_brand"
                                               id="brand-{{$brand->slug}}" value="{{$brand->slug}}"
                                               class="w-4 h-4 mr-2">
                        ...
                        <ul>
                            <li class="mb-4">
                                <label for="featured" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" id="featured" value="false" wire:model.live="featured"
                                           class="w-4 h-4 mr-2">
                                    <span class="text-lg dark:text-gray-400">Featured Stock</span>
                                </label>
                            </li>
                            <li class="mb-4">
                                <label for="sale" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" id="sale" value="false" wire:model.live="sale"
                                           class="w-4 h-4 mr-2">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div>
                            <div class="font-semibold">{{Number::currency($price_range, in:'usd')}}</div>
                            <input type="range" wire:model.live="price_range"
                                   class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer" min="50"
                                   max="10000" value="3000" step="100">
                            <div class="flex justify-between ">
                                <span class="inline-block text-lg font-bold text-blue-400 ">
                                    {{Number::currency(50, in:'usd')}}
                                </span>
                                <span class="inline-block text-lg font-bold text-blue-400 ">
                                    {{Number::currency(10000, in:'usd')}}
                                </span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{$product->id}}">
                                <div class="border border-gray-300 dark:border-gray-700">
                                    <div class="relative bg-gray-200">
                                        <a wire:navigate href="{{route('products.show',$product)}}" class="">
                                            <img src="{{url('storage', $product->images[0])}}" alt=""
                                                 class="object-fit w-full h-56 mx-auto ">
                                        </a>
                                    </div>
                                    <div class="p-3 ">
                                        ...
                                        <p class="text-lg ">
                                            <span
                                                class="text-green-600 dark:text-green-600">{{Number::currency($product->price, in: 'usd')}}</span>
                                        </p>
                                    </div>
                                    <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">
                                        <a wire:click.prevent="addProductToCart({{$product->id}})" href=""
